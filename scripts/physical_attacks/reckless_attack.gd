...
func use(opponent_stats=null, self_stats=null, battle=null):
	...
	var dmg = max(0, self_stats["atk"] - opponent_stats["def"]) + 3
	dmg = dmg * opponent_stats["next_dmg_taken_modifier"] * 2
	opponent_stats["hp"] = max(0, opponent_stats["hp"] - dmg)
	battle.update_all_progress_bars()
	
	self_stats["next_dmg_taken_modifier"] = self_stats["next_dmg_taken_modifier"] * 2
	
	if (self_stats == battle.current_player_stats):
		battle.animation_player.play("reckless_attack")
...
func get_description():
	return "Deal double damage, but also take double damage this turn."
